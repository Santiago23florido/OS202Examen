...
class Visualizer3D:
    ...
    def __init__(self, points, colors, luminosities, bounds):
        ...
        try:
            ...
            self.vbo_vertices = None
            self.vbo_colors = None
            print(f"OpenGL VBO indisponible, bascule en mode compatible: {exc}")
    ...
    def _init_sdl(self):
        ...
        if sdl2.SDL_GL_MakeCurrent(self.window, self.gl_context) != 0:
            raise RuntimeError(f"Erreur activation contexte GL: {sdl2.SDL_GetError()}")
    ...
    def _render(self):
        ...
        if self.use_vbo:
            try:
                ...
                self.use_vbo = False
                glBindBuffer(GL_ARRAY_BUFFER, 0)
                print(f"Rendu VBO indisponible, bascule en mode compatible: {exc}")
    ...
    def run(self, updater=None, dt = 0.001, warmup_frames: int = 5, report_every: int = 60 ):
        """
        ...
        while self.running:
            ...
            #sdl2.SDL_Delay(10)
            frame_count += 1
            render_ms = 1_000.0 * (render_end - render_start)
            update_ms = 1_000.0 * (update_end - update_start)
            ...
            avg_render_ms = total_render_time / measured_frames if measured_frames > 0 else render_ms
            avg_update_ms = total_update_time / measured_frames if measured_frames > 0 else update_ms

            print(
                f"Frame {frame_count:5d} | render={render_ms:8.3f} ms | update={update_ms:8.3f} ms | "
                f"avg_render={avg_render_ms:8.3f} ms | avg_update={avg_update_ms:8.3f} ms",
                end='\r'
            )

            if measured_frames > 0 and measured_frames % report_every == 0:
                print()
                print(
                    f"[timing] {measured_frames} frames mesurees | "
                    f"avg_render={avg_render_ms:.3f} ms | avg_update={avg_update_ms:.3f} ms"
                )
        # Nettoyage
        print()
        if measured_frames > 0:
            print(
                f"[timing final] {measured_frames} frames mesurees | "
                f"avg_render={total_render_time / measured_frames:.3f} ms | "
                f"avg_update={total_update_time / measured_frames:.3f} ms"
            )
        self.cleanup()
